...
vec3 discColor(vec3 baseColor, vec3 planarDiscPos, vec3 discDir, vec3 cameraPos, float u, float radius)
{
	...
	intensity += dopplerDistance * DiscSpeed * DopplerBeamingFactor;
...
void main()
{
	vec3 posOS = posMS.xyz + Center;

	// Initial ray information
	vec3 rayOrigin = vec3(0, 0, 0);
	vec3 rayDir = normalize(posOS.xyz - rayOrigin);
	...
	{
		float discDst = intersectDisc(rayOrigin, rayDir, p1, p2, DiscDirection, discRadius, innerRadius);
		if(discDst < maxFloat)
		{
			transmittance = 1;
	...
	float texCol = texture(Sampler0, uv * tiling).r;

	vec2 screenUV = gl_FragCoord.xy / ScreenSize.xy;
	vec3 bgCol = texture(DiffuseSampler, screenUV).rgb;
